...
from app.schemas.external import ExternalConsultationSchema
...
class ExternalRepository:
    ...
    async def recognize_calories_from_text(
    ...
    async def edit_meal_text(
        self, meal_json: str, text: str, language: str
    ) -> list[dict]:
        response = await self.deepseek_client.chat.completions.create(
            model="deepseek-chat",
            messages=[
    ...
    async def edit_sport_text(
        self, sport_json: str, text: str, language: str
    ) -> list[dict]:
        response = await self.deepseek_client.chat.completions.create(
            model="deepseek-chat",
            messages=[
    ...
    async def get_consulation(
        self, schema: ExternalConsultationSchema
    ) -> str:
        response = await self.deepseek_client.chat.completions.create(
            model="deepseek-chat",
            messages=[
